"""
LSTM trainer: 2-layer LSTM with dropout for time-series stock prediction.
...
if _PT_AVAILABLE:
    ...
    class _LSTMModel(nn.Module):
        """2-layer LSTM followed by a linear head."""
    ...
    class LSTMTrainer(BaseModel, TorchTrainerMixin):
        """LSTM model trainer."""
        ...
        def load(self, path: str) -> nn.Module:
            raise NotImplementedError(
                "LSTM model loading requires n_features; use LSTMTrainer.fit() instead"
            )

else:
    class LSTMTrainer:
        def __init__(self):
            raise RuntimeError("PyTorch is required for LSTMTrainer.")
